 ...
 */
export async function validateAndEnhancePrompt(
  prompt: string,
  ...
  try {
    const param = await ssm.send(
      new GetParameterCommand({ Name: '/EpicWeave/mythology/allowed-types' })
    );
 ...
 */
export function hasLocalMythologyReference(prompt: string): boolean {
  const lower = prompt.toLowerCase();
  return (
    HINDU_KEYWORDS.some(k => lower.includes(k)) ||
