...
const handler = async (event) => {
    try {
        ...
        if (!userEmail) {
            return (0, types_1.ERROR_RESPONSE)(400, 'User email not available');
        }
        // Build email
        const itemsList = orderItems.map((item) => `• ${item.name} (${item.size}/${item.color}) x${item.quantity} — $${item.lineTotal.toFixed(2)}`).join('\n');
        const emailBody = `
...
<body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #888; font-size: 14px; margin-top: 30px;">
    ...
    Thank you for shopping with EpicWeave.
        ...
        return (0, types_1.ERROR_RESPONSE)(500, 'Failed to send confirmation email');
    }
};
