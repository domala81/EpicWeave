...
Given(
  "I have a pre-designed product {string} in my cart",
  async function (this: EpicWeaveWorld, name: string) {
    this.attach(`Pre-designed product "${name}" in cart`);
  },
);

Given(
  "I have a custom design in my cart from session {string}",
  async function (this: EpicWeaveWorld, sessionId: string) {
    this.attach(`Custom design from session ${sessionId} in cart`);
  },
);

Given(
  "the product variant {string} has {int} units in stock",
  async function (this: EpicWeaveWorld, variant: string, stock: number) {
...
// Add to cart
When(
  "I add a pre-designed product to my cart with:",
...
When(
  "I add a custom design to my cart with:",
  async function (this: EpicWeaveWorld, table: any) {
    const rows = table.rowsHash();
...
When(
  "I add the same product with same size and color again",
...
Then(
  "a CartItem should be created in DynamoDB with PK {string}",
  async function (this: EpicWeaveWorld, pk: string) {
    this.attach(`CartItem created with PK: ${pk}`);
  },
);

Then(
  "the existing item quantity should increase",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the session status should be updated to {string}",
...
// Quantity management
When(
  "I update the quantity of item {string} to {int}",
  async function (this: EpicWeaveWorld, itemId: string, qty: number) {
    this.attach(`Updating ${itemId} quantity to ${qty}`);
  },
);

When(
  "I remove item {string} from my cart",
  async function (this: EpicWeaveWorld, itemId: string) {
    this.attach(`Removing ${itemId} from cart`);
  },
);

Then(
  "the cart item quantity should be {int}",
...
// 'I should see an error {string}' is defined in auth.steps.ts (shared)
...
Then(
  "I should see {int} items in my cart",
...
Then(
  "the subtotal should be ${float}",
  async function (this: EpicWeaveWorld, amount: number) {
    this.attach(`Subtotal: $${amount}`);
  },
);

Then(
  "custom items should show quantity as {int} and not be editable",
  async function (this: EpicWeaveWorld, qty: number) {
...
// Out of stock
When(
  "I try to add an out-of-stock product to my cart",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the Lambda should check variant stockCount in DynamoDB",
  async function (this: EpicWeaveWorld) {
...
// Cart persistence
Given(
  "I added items to my cart yesterday",
  async function (this: EpicWeaveWorld) {
...
Then(
  "my cart items should still be there",
  async function (this: EpicWeaveWorld) {
...
Given(
  "I have {int} items in my cart totaling ${float}",
  async function (this: EpicWeaveWorld, count: number, total: number) {
...
// Shipping
When(
  "I enter a valid US shipping address:",
  async function (this: EpicWeaveWorld, table: any) {
    const rows = table.rowsHash();
...
When(
  "I enter a non-US shipping address",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the shipping address form should only allow US addresses",
  async function (this: EpicWeaveWorld) {
...
// Shipping cost
Then(
  "the flat-rate shipping should be ${float}",
  async function (this: EpicWeaveWorld, rate: number) {
    this.attach(`Flat rate shipping: $${rate}`);
  },
);

Then(
  "each additional item should add ${float} to shipping",
  async function (this: EpicWeaveWorld, rate: number) {
...
Then(
  "total shipping should be ${float}",
  async function (this: EpicWeaveWorld, total: number) {
    this.attach(`Total shipping: $${total}`);
  },
);

Then(
  "shipping rate should be read from Parameter Store",
  async function (this: EpicWeaveWorld) {
...
// Payment
When(
  "I enter valid Stripe payment details",
  async function (this: EpicWeaveWorld) {
    this.attach("Stripe payment details entered");
  },
);

// 'I click "Place Order"' defined in shared.steps.ts
...
Then(
  "a Stripe PaymentIntent should be created for the order total",
  async function (this: EpicWeaveWorld) {
...
Then(
  "a DynamoDB TransactWriteItems should atomically:",
  async function (this: EpicWeaveWorld, table: any) {
...
Then(
  "an Order record should be created with status {string}",
  async function (this: EpicWeaveWorld, status: string) {
...
Then(
  "order items should be saved under PK ORDER#{string}",
...
Then(
  "stock should be decremented for pre-designed items",
  async function (this: EpicWeaveWorld) {
...
Given(
  "another customer purchased the last unit of a product in my cart",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the transaction should fail with ConditionExpression error",
  async function (this: EpicWeaveWorld) {
...
Then(
  "a confirmation email should be sent via SES",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the email should contain order ID, items, total, and shipping address",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the order confirmation page should display a success checkmark",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the order details with items and totals should be shown",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the session fee should be marked as non-refundable",
  async function (this: EpicWeaveWorld) {
...
Then(
  "the checkout page should display {string}",
  async function (this: EpicWeaveWorld, text: string) {
